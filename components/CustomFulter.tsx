'use client'
import { carOptionsProps } from '@/types'
...
const CustomFulter = ({title,options}:carOptionsProps) => {
  const router= useRouter()
  const [selected, setselected] = useState(options[0]);
  const handleUpdateParams=(e:{title:string,value:string})=>{
  const updatedPath=updateSearchParams(title,e.value.toLowerCase())
